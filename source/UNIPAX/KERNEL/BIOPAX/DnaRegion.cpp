...
#include<set>
//Begin section for file DnaRegion.cpp
//TODO: Add definitions that you want preserved
//End section for file DnaRegion.cpp

UniPAX::DnaRegion::DnaRegion() : UniPAX::PhysicalEntity()
//, entityReference(0)
//, id_entityReference(0)
{
}

UniPAX::DnaRegion::DnaRegion(const DnaRegion & arg) : UniPAX::PhysicalEntity(arg),
		entityReference(arg.entityReference)
//		id_entityReference(arg.id_entityReference)
{}

UniPAX::DnaRegion & UniPAX::DnaRegion::operator =(const DnaRegion & arg)  {
...
UniPAX::DnaRegion::~DnaRegion()  {
}

bool UniPAX::DnaRegion::merge(DnaRegion& object)
{
	{
...
bool UniPAX::DnaRegion::update(PersistenceManager& manager)
{
	{
...
const std::vector<boost::shared_ptr<UniPAX::EntityReference> > & UniPAX::DnaRegion::getEntityReferences() const {
	return entityReference;
}

std::vector<boost::shared_ptr<UniPAX::EntityReference> > & UniPAX::DnaRegion::getEntityReferences() {
	return entityReference;
}

void UniPAX::DnaRegion::setEntityReferences(std::vector<boost::shared_ptr<UniPAX::EntityReference> > _entityReferences) {
	entityReference = _entityReferences;
}

void UniPAX::DnaRegion::addEntityReference(boost::shared_ptr<UniPAX::EntityReference>  _entityReference) {
	entityReference.push_back(_entityReference);
}

bool UniPAX::DnaRegion::setAttribute(std::string& attribute, std::string& value, PersistenceManager& manager) {
	...
	if (boost::iequals(attribute,"entityReference"))
	{
		...
		if (object == 0)
		{
			std::cerr << "DnaRegion::setAttribute - object not known (value = " << value << ")" << std::endl;
			return false;
		}
...
bool UniPAX::DnaRegion::getAttribute(std::vector<std::pair<std::string,std::string> >& value, PersistenceManager& manager) {
...
//void UniPAX::DnaRegion::init (odb::callback_event e, odb::database& db) const
//{
