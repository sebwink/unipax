...
UniPAX::oracle::OracleODBLoadRunnable::OracleODBLoadRunnable(std::vector<boost::shared_ptr<UniPAX::UPBase> > & result, std::vector<UnipaxId> & _ids) : UniPAX::ODBLoadRunnable(result, _ids)
{
}


UniPAX::oracle::OracleODBLoadRunnable::~OracleODBLoadRunnable()
{
}


void UniPAX::oracle::OracleODBLoadRunnable::run(boost::shared_ptr<odb::database> database)
{
	try
	{
		...
		for (std::vector<UnipaxId>::const_iterator it(ids.begin()); it != ids.end(); it++)
		{
			...
			std::string type = UniPAX::ODBWorker::getType(*it);
	...
	{
		std::cerr << "OracleIDBLoadRunnable::getObjectByID: odb_exception - " << e.what() << std::endl;
	}
