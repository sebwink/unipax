...
UniPAX::mysql::MySQLODBPersistRunnable::MySQLODBPersistRunnable(boost::shared_ptr<UniPAX::UPBase> _object) : UniPAX::ODBPersistRunnable(_object)
{
}

UniPAX::mysql::MySQLODBPersistRunnable::MySQLODBPersistRunnable(std::vector<boost::shared_ptr<UniPAX::UPBase> > _objects) : UniPAX::ODBPersistRunnable(_objects)
{
}

UniPAX::mysql::MySQLODBPersistRunnable::~MySQLODBPersistRunnable()
{
}

void UniPAX::mysql::MySQLODBPersistRunnable::run(boost::shared_ptr<odb::session> session, boost::shared_ptr<odb::database> database)
{
	unsigned short max_retry = 5;
	...
	try
	{
		...
		for (std::vector<boost::shared_ptr<UniPAX::UPBase> >::const_iterator it(objects.begin()); it != objects.end(); it++, i++)
		{
...
//			}
	...
	catch (const odb::recoverable& e)
	{
		...
		{
		  std::cerr << "MySQLODBPersistRunnable::persist: Retry limit exceeded." << std::endl;
			return;
		}
		std::cerr << "MySQLODBPersistRunnable::persist: odb_exception - " << e.what() << "\nRetrying..." << std::endl;
		continue;
	}
	catch (const odb::exception& e)
	{
		std::cerr << "MySQLODBPersistRunnable::persist: odb_exception - " << e.what() << std::endl;
		return;
	}
  }
