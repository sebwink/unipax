...
BIOPAXReaderHandler::BIOPAXReaderHandler() : debug(false), objects_only(false), errorOnInconsistentData(true)
{
}

/*! \brief destructor */
BIOPAXReaderHandler::~BIOPAXReaderHandler()
{
}


UniPAX::BIOPAXReaderHandler::BIOPAXReaderHandler(const BIOPAXReaderHandler & arg)  :
		manager(arg.manager),
...
UniPAX::BIOPAXReaderHandler & UniPAX::BIOPAXReaderHandler::operator =(const BIOPAXReaderHandler & arg)
{
	manager = arg.manager;
	tree_structure = arg.tree_structure;
	object_tree_structure = arg.object_tree_structure;
...
bool BIOPAXReaderHandler::startElement(const QString & namespaceURI, const QString & localName, const QString & qName, const QXmlAttributes & attributes)
{
...
//	std::cout << "startElement : " << current_full_tag << std::endl;
	...
	boost::shared_ptr<UniPAX::UPBase> current_object;
	...
	if (objects_only)
	{
		for (int index = 0; index < attributes.length(); index++)
		{
			...
			if (attr_name == "rdf:ID" || attr_name == "rdf:about")
			{
				...
				if (!current_object)
				{
					std::cerr << "ERROR: Instance was not created: id - " << attr_value << " , object - " << current_tag << std::endl;
					return false;
				}
			}
		}
	...
	for (int index = 0; index < attributes.length(); index++)
	{
		...
		else if (attr_name == "rdf:resource")
		{
			...
			if (!manager.setAttribute(id_structure.back(), current_tag, attr_value))
			{
				...
				if (errorOnInconsistentData)
				{
					std::cerr << "ERROR: Attribute was not set: id - " << id_structure.back() << " , attribute - " << current_tag << " , value - " << attr_value << std::endl;
					return false;
				} else
				{
					std::cerr << "WARNING: Attribute was not set: id - " << id_structure.back() << " , attribute - " << current_tag << " , value - " << attr_value << std::endl;
				}
			}
...
//	std::cerr << "BIOPAXReaderHandler: Found " << serialize_stack(status) << " as element and don't know how to handle..." << std::endl;

	return true;
}

bool BIOPAXReaderHandler::characters(const QString & chars)
{
...
bool BIOPAXReaderHandler::endElement( const QString& namespaceURI, const QString& localName, const QString& qName)
{
	std::string current_full_tag(qName.toStdString());
	std::string current_tag(localName.toStdString());
...
//		std::cout << current_full_tag.substr(0,3) << std::endl;
	...
	if (!attribute_structure.empty() && attribute_structure.back() == current_tag)
	{
		...
		if (!manager.setAttribute(id_structure.back(), current_tag, current_data))
		{
			...
			if (errorOnInconsistentData)
			{
				std::cerr << "ERROR: Attribute was not set: id - " << id_structure.back() << " , attribute - " << current_tag << " , value - " << current_data << std::endl;
				return false;
			}
			else
			{
				std::cerr << "WARNING: Attribute was not set: id - " << id_structure.back() << " , attribute - " << current_tag << " , value - " << current_data << std::endl;
			}
		}
		if (debug) std::cout << "done" << std::endl;
...
bool BIOPAXReaderHandler::endDocument()
{
	return true;
}
...
bool BIOPAXReaderHandler::warning(const QXmlParseException & exception)
{
	std::string message(exception.message().toStdString());
	std::string filename(exception.systemId().toStdString());

	std::cerr << "BIOPAXReaderHandler - Warning: " << message
			<< ", at file: " << filename
...
bool BIOPAXReaderHandler::error(const QXmlParseException & exception)
{
	std::string message(exception.message().toStdString());
	std::string filename(exception.systemId().toStdString());
...
bool BIOPAXReaderHandler::fatalError(const QXmlParseException & exception)
{
	std::string message(exception.message().toStdString());
	std::string filename(exception.systemId().toStdString());
...
unsigned long BIOPAXReaderHandler::string_to_long(std::string text)
{
...
std::string BIOPAXReaderHandler::cleanString(std::string s)
{
...
void BIOPAXReaderHandler::setDebug(bool debug_)
{
	debug = debug_;
}

void BIOPAXReaderHandler::setErrorOnInconsistentData(bool errorOnInconsistentData_)
{
	errorOnInconsistentData = errorOnInconsistentData_;
}

void BIOPAXReaderHandler::createObjectsOnly(bool objects_only_)
{
	objects_only = objects_only_;
}

UniPAX::PersistenceManager& BIOPAXReaderHandler::getPersistenceManager()
{
	return manager;
}

const UniPAX::PersistenceManager& BIOPAXReaderHandler::getPersistenceManager() const
{
	return manager;
}
